...
const authCtrl = {
    ...
    generateAccessToken : async(req ,res) => {
        try{
            ...
            jwt.verify(rf_token, process.env.REFRESHTOKENSECRET, async(err, infos)=>{
                if (err) return res.status(400).json({msg : 'no'});
                const user = await Users.findById(infos.id).select("-password").populate("friends following")

                if(!user) return res.status(400).json({msg:"user does not exist"})
                
                const access_token = createAccessToken({id : infos.id});
